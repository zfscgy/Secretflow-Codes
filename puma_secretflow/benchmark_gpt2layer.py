...
config = GPT2Config()
# Modify the transformer size. The inner dimension of MLP will auto-change to 4 * n_embd
config.n_embd = 768
config.n_head = 12

transformer = FlaxGPT2Block(config)


def transformer_inference(input_tensor, params):
    return transformer.apply(params, input_tensor)

_, initial_params = transformer.init_with_output(jax.random.PRNGKey(1926), jnp.ones([1, 1, transformer.config.n_embd]))


np.random.seed(8964)
input_tensor = jnp.array(np.random.normal(0, 1, [1, 1, transformer.config.n_embd]))

print("Expected input:", transformer.apply(initial_params, input_tensor)[0][0, 0, :3])
...
device = spu
model_params_ct, input_tensor_ct = model_params.to(device), input_tensor.to(device)

for i in range(5):
    print("=============================================\n\n\n\n=================================")
    with hack_softmax_context("hijack jax softmax", enabled=True), hack_gelu_context("hack jax gelu", enabled=True):
        output_ct = spu(transformer_inference, copts=copts)(
            input_tensor_ct, model_params_ct
        )

    print(sf.reveal(output_ct)[0][0, 0, :3])
